...
async def main():
    ...
    try:
        ...
        # We use a pattern that has some repetition but is filled to size
        print("Generating dummy chunk data...")
        chunk_data = b"NETCONDUIT_BENCHMARK_BLOCK_" * (chunk_size_bytes // 32)
        # Ensure exact size
        ...
        for i in range(total_chunks):
            ...
            # Send using the direct binary stream (blocks until written to QUIC stream buffer)
